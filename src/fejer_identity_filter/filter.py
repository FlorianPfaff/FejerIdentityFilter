"""PyRecEst-style Fejér identity Fourier filter."""
...
from .fejer import normalize_coefficient_shape


class FejerIdentityFilter(AbstractFilter, HypertoroidalFilterMixin):
    """Hypertoroidal identity Fourier filter with Fejér update reduction.
    ...
    def __init__(self, n_coefficients: int | tuple[int, ...]):
        ...
        AbstractFilter.__init__(self, FejerHypertoroidalFourierDistribution(coeff_mat))
    ...
    def filter_state(self, new_state):
        if isinstance(new_state, FejerHypertoroidalFourierDistribution):
            if new_state.coeff_mat.shape != self._filter_state.coeff_mat.shape:
                warnings.warn(
                    "setState:noOfCoeffsDiffer: New density has a different number of coefficients; applying Fejer reduction to the filter shape.",
                    RuntimeWarning,
                )
                new_state = new_state.fejer_reduce(self._filter_state.coeff_mat.shape)
        ...
        if isinstance(new_state, HypertoroidalFourierDistribution):
            ...
            warnings.warn(
                "setState:nonFejerFourier: converting identity Fourier distribution to Fejer identity form.",
                RuntimeWarning,
            )
    ...
    def predict_identity(self, d_sys):
        ...
        d_sys = self._as_fejer_identity_distribution(
            d_sys,
            "predict_identity:automaticConversion: d_sys is not a Fejer identity Fourier distribution. Transforming with the same number of coefficients as the filter.",
        )
    ...
    def get_f_trans_as_hfd(self, f, noise_distribution):
        """Build a Fejér identity transition density for nonlinear prediction."""
        ...
        with warnings.catch_warnings():
            ...
            return FejerHypertoroidalFourierDistribution.from_function(_f_trans, n_coefficients_2d, apply_fejer=True)
    ...
    def predict_nonlinear_via_transition_density(self, f_trans):
        ...
        with warnings.catch_warnings():
            ...
            self._filter_state = FejerHypertoroidalFourierDistribution(c_predicted)
    ...
    def update_identity(self, d_meas, z):
        ...
        d_meas = self._as_fejer_identity_distribution(
            d_meas,
            "update_identity:automaticConversion: d_meas is not a Fejer identity Fourier distribution. Transforming with the same number of coefficients as the filter.",
        )
        z = array(z)
    ...
    def update_nonlinear(self, likelihood, z=None):
        ...
        if z is None:
            ...
            likelihood = self._as_fejer_identity_distribution(likelihood, "update_nonlinear:nonFejerFourier: converting likelihood to Fejer identity form.")
        else:
    ...
    def _as_fejer_identity_distribution(self, distribution, warning_message: str) -> FejerHypertoroidalFourierDistribution:
        if isinstance(distribution, FejerHypertoroidalFourierDistribution):
            if distribution.coeff_mat.shape == self._filter_state.coeff_mat.shape:
                return distribution
            warnings.warn(warning_message, RuntimeWarning)
            return distribution.fejer_reduce(self._filter_state.coeff_mat.shape)
